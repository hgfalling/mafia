        ...
        for detective_peek in detective_peeks:
            for bg_protect in bg_protects:
                if detective_peek is None and bg_protect is None:
                    ...
                    outcomes[(mafia_kill,detective_peek,bg_protect)] = Gamestate(gs.day+1,0,None,new_players)
                    ...
                    if detective_peek != mafia_kill:
                        ...
                        new_players = gs.players + kill_change(mafia_kill)
                        outcomes[(mafia_kill,detective_peek,bg_protect,0)] = Gamestate(gs.day+1,0,None,new_players)
                        #if there are more than 1, they could miss each other:
                        if gs.players[mafia_kill] >= 2:
                            new_players = gs.players + kill_change(mafia_kill)
                            new_players = new_players + peek_change(detective_peek)
                            outcomes[(mafia_kill,detective_peek,bg_protect,1)] = Gamestate(gs.day+1,0,None,new_players)
                elif detective_peek is None:
                    #then there's no peek, just figure out BG.
                    if bg_protect != mafia_kill:
                        new_players = gs.players + kill_change(mafia_kill)
                        outcomes[(mafia_kill,detective_peek,bg_protect)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                    else:
                        #bg could save:
                        new_players = gs.players + verified_change(mafia_kill)
                        outcomes[(mafia_kill,detective_peek,bg_protect,2)] = Gamestate(gs.day+1,0,verified_version(bg_protect),
                                                                                       new_players)
                        if gs.players[mafia_kill] >= 2:
                            new_players = gs.players + kill_change(mafia_kill)
                            outcomes[(mafia_kill,detective_peek,bg_protect,2)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                else: #a kill,a peek,a protect.
                    if len(set[mafia_kill,detective_peek,bg_protect]) == 3: #all different
                        new_players = gs.players + kill_change(mafia_kill)
                        new_players = new_players + peek_change(detective_peek)
                        outcomes[(mafia_kill,detective_peek,bg_protect)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                    if len(set[mafia_kill,detective_peek,bg_protect]) == 2: #one different, two the same
                        if mafia_kill == detective_peek:
                            new_players = gs.players + kill_change(mafia_kill)
                            outcomes[(mafia_kill,detective_peek,bg_protect,0)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                            if gs.players[mafia_kill] >= 2:
                                new_players = gs.players + kill_change(mafia_kill)
                                new_players = new_players + peek_change(detective_peek)
                                outcomes[(mafia_kill,detective_peek,bg_protect,1)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                        if mafia_kill == bg_protect:
                            new_players = gs.players + verified_change(mafia_kill)
                            new_players = gs.players + peek_change(detective_peek)
                            outcomes[(mafia_kill,detective_peek,bg_protect,0)] = Gamestate(gs.day+1,0,verified_version(bg_protect),new_players)
                            if gs.players[mafia_kill] >= 2:
                                new_players = gs.players + kill_change(mafia_kill)
                                new_players = gs.players + peek_change(detective_peek)
                                outcomes[(mafia_kill,detective_peek,bg_protect,1)] = Gamestate(gs.day+1,0,bg_protect,new_players)
                        if bg_protect == detective_peek:
                            ...
                            if gs.players[bg_protect] >= 2:
                                new_players = gs.players + kill_change(mafia_kill)
                                new_players = gs.players + peek_change(detective_peek)
                                outcomes[(mafia_kill,detective_peek,bg_protect,1)] = Gamestate(gs.day+1,0,peeked_version(bg_protect),new_players)
                    if len(set[mafia_kill,detective_peek,bg_protect]) == 1: #all the same
                        ...
                        if n >= 2:
                            ...
                            new_players = gs.players + verified_change(mafia_kill)
                            new_players = gs.players + peek_change(detective_peek)
                            outcomes[(mafia_kill,detective_peek,bg_protect,2)] = Gamestate(gs.day+1,0,verified_version(bg_protect),new_players)
                            #3: 011 (so bg protects peeked player and mafia kills.)
                            new_players = gs.players + verified_change(mafia_kill)
                            new_players = gs.players + peek_change(mafia_kill)
                            outcomes[(mafia_kill,detective_peek,bg_protect,3)] = Gamestate(gs.day+1,0,peeked_version(bg_protect),new_players)
                        if n >= 3:
                            #4: 012 (so all 3 different)
                            new_players = gs.players + verified_change(mafia_kill)
                            new_players = gs.players + peek_change(mafia_kill)
                            outcomes[(mafia_kill,detective_peek,bg_protect,4)] = Gamestate(gs.day+1,0,bg_protect,new_players)
    ...
    for key,val in outcomes.items():
        if not detective_alive(val):
            ...
            for ptype in peeked_types():
                ...
                gsp[ptype] = 0
            outcomes[key] = Gamestate(day=val.day,time=val.time,last_protected=val.last_protected,players=Players(*tuple(gsp)))
    ...
    for node in tree.all_nodes():
        ...
        for key,val in outcomes.items():
            if key == "Detective Out":
                tree.create_node(f"DO {str(val)}",node_id + "_" + "DO",node_id,data=(key,val,False))    
            else:
                s = "".join([to_string(x) for x in key[0:3]]) + (str(key[-1]) if len(key) > 3 else "")
                tree.create_node(f"{s} {str(val)}",node_id + "_" + s,node_id,data=(key,val,False))
...
def expand_night_nodes(tree:treelib.Tree):
    ...
    for node in tree.all_nodes():
        ...
        for key,val in outcomes.items():
            s = "".join([to_string(x) for x in key[0:3]]) + (str(key[-1]) if len(key) > 3 else "")
            tree.create_node(f"{s} {str(val)}",node_id + "_" + s,node_id,data=(key,val,False,0.0))
...
def make_game(pl:Players,gs:Gamestate):
    ...
    while unexpanded_nodes(g) > 0:
        ...
        if len(gss) > 0:
            ...
            new_game.create_node(f'Day {new_gs.day}','root' + str(new_gs.day-1),data=(None,new_gs,True,1.0))
            for idx,ugss in enumerate(gss):
                new_game.create_node(f'{str(ugss)}',"root" + str(new_gs.day-1) + "_n" + str(idx),
                                     parent=new_game.root,data=(None,ugss,False,0.0))
    ...
    for idx, t in enumerate(games):
        ...
        for node in t.leaves():
            if winner(node.data[1]) != 0:
                ...
                target = [x.identifier for x in games[idx+1].children(games[idx+1].root) if x.data[1] == node.data[1]][0]
